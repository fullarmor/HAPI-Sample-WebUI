app.controller('DeviceFilesController', function ($scope, $compile, $http, $location) {
    $scope.FilesAndFolders = [];
    $scope.GetFilesAndFolders = function () {
        var urlSuffix = "/route/hapi/Share/GetFilesAndFolders"; // Note: individual "Share", not         
    ...
    $scope.GetFolders = function (fileIdentifier, progressLoaderClass) {
        ...
        var urlSuffix = "/route/hapi/Directory/GetFilesAndFolders"; // Note: individual "Share", not "Shares"
        _Url = _hapiServer + urlSuffix;
    ...
    $scope.promptNewFolder = function() {
        ...
        var buttons = $('<div class="text-center"><a href="index.html" class="btn btn-primary" onclick="CreateFolder(); return false;">Create</a><a href="#" class="btn btn-primary" onclick="CloseModalBox();">Cancel</a></div>');
    ...
    $scope.promptDeleteSelectedItems = function () {
        ...
        if (contextAgentId > -1)
            delUrl = _hapiServer + "/route/agent/" + contextAgentId + "/Share/DeleteFiles";
        
        var buttons = $('<div class="text-center"><a href="index_v1.html" class="btn btn-primary" onclick="DeleteSelectedItems(\'' + delUrl + '\',' + '\'#DeviceFilesController\'); return false;">Delete</a><a href="#" class="btn btn-primary" onclick="CloseModalBox();">Cancel</a></div>');         
        OpenModalBox(header, form, buttons);            
    };
    ...
    $scope.promptShareFileAsLink = function() {
        ...
        var form = $('<div class="form-group"><label class="control-label">' + message + '</label></div>');
        var buttons = $('<div class="text-center"><a href="index.html" class="btn btn-primary" onclick="ShareSelectedItem(); return false;">Share</a><a href="#" class="btn btn-primary" onclick="CloseModalBox();">Cancel</a></div>');
        OpenModalBox(header, form, buttons);            
    }
    ...
    $scope.downloadItem = function(){
        var urlSuffix;
        if (contextAgentId > -1)
            dlurl = _hapiServer + "/route/agent/" + contextAgentId + "/Share/Download?FileIdentifier=" + encodeURIComponent($scope.selectedItem.FileIdentifier);
        else
            dlurl = _hapiServer + "/route/hapi/Share/Download?FileIdentifier=" + encodeURIComponent($scope.selectedItem.FileIdentifier);        
